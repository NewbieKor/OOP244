...
namespace sdds {
	...
	void CleanerBot::display() const
	{
		...
		cout << "| " << setw(10) << left << m_location << " | " << setw(7) << right << m_battery << " | " << setw(18) << m_brush << " | " << setw(6) << left << (m_isActive ? "YES" : "NO")<<" |"<< endl;
	...
	int report(CleanerBot* bot, short num_bots)
	{
		...
		for (short i = 0; i < num_bots; i++) {
			...
			if (bot[i].isValid() && bot[i].getBattery() < 30) lowBatteryCount++;
		}
		cout << endl;
