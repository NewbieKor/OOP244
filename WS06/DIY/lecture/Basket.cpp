...
namespace sdds {
	...
	Basket& Basket::operator=(const Basket& other) // &는 레퍼런스.
	{
		if (this != &other) { // 주소값 비교 (여기서 &는 주소). 셀프 체크
	...
	Basket& Basket::operator+=(Fruit fruit) // 중요
	{
		Fruit* temp = new Fruit[m_cnt + 1];
		for (int i = 0; i < m_cnt; i++) {
			temp[i] = m_fruits[i];
		}
		temp[m_cnt++] = fruit; // 마지막 요소에 fruit 삽입.
		delete[] m_fruits;
		m_fruits = temp;
		return *this;
	}
